...
static inline cxl_block *cxl_page_block_at(void *page_start,
                                           uint64_t block_size, uint64_t i) {
  return (cxl_block *)((char *)page_start + (i * block_size));
}

static inline uint64_t pack_machine_process_id(uint64_t machine_id,
                                               uint16_t process_id) {
  return (machine_id << 16) + process_id;
}

static inline uint64_t pack_ref_info(uint64_t lcid, uint64_t ref_cnt,
                                     uint32_t lenum) {
